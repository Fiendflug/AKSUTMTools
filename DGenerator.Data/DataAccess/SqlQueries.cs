...
namespace DGenerator.Data.DataAccess
{
    public class SqlQueries
    {
        ...
        public SqlQueries()
        {
            ...
            GetCivilUsersForDataGrid = string.Format("SELECT users_accounts.account_id, users.login, users.full_name, users.actual_address, users.flat_number FROM users, users_accounts, account_tariff_link WHERE users.id = users_accounts.uid AND users_accounts.account_id = account_tariff_link.account_id AND account_tariff_link.tariff_id = 66 AND users.is_deleted = 0 AND account_tariff_link.is_deleted = 0;");
            GetLegalUsersForDataGrid = string.Format("SELECT users_accounts.account_id, users.login, users.full_name, users.actual_address, users.flat_number FROM users, users_accounts, account_tariff_link WHERE users.id = users_accounts.uid AND users_accounts.account_id = account_tariff_link.account_id AND account_tariff_link.tariff_id = 65 AND users.is_deleted = 0 AND account_tariff_link.is_deleted = 0;");
